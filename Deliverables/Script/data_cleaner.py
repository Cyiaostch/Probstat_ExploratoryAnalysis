...
# # Select Only Importants Columns
...
data=data[[
            ...
            'Pelajar / Mahasiswa','Ibu Rumah Tangga','Pegawai Negeri','Profesi ( Dokter, Dosen, Pengacara, dll)','Karyawan Swasta','Pengusaha',
            'Laki-Laki','Perempuan',
            ...
            'Gojek - pernah','Gojek - tidak pernah','Uber motor - pernah','Uber motor - tidak pernah','Grabbike - pernah','Grabbike - tidak pernah','GoCar - pernah','GoCar - tidak pernah','Uber Car - pernah','Uber Car - tidak pernah','Grab Taxi - pernah','Grab Taxi - tidak pernah','Grab Car - pernah','Grab Car - tidak pernah'
         ]]


# # Constructing New Columns(Features)
...
for i in data.index:
    ...
    if(data['Pelajar / Mahasiswa'][i] not in [np.nan]):
        ...
        data['Profesi'][i]=2
    elif(data['Pegawai Negeri'][i] not in [np.nan]):
        data['Profesi'][i]=3
    elif(data['Profesi ( Dokter, Dosen, Pengacara, dll)'][i] not in [np.nan]):
        data['Profesi'][i]=3
    elif(data['Karyawan Swasta'][i] not in [np.nan]):
        data['Profesi'][i]=3
    elif(data['Pengusaha'][i] not in [np.nan]):
        data['Profesi'][i]=3
    else:
        data['Profesi'][i]=0
...
data['Gojek Motor']=np.nan
data['Grab Motor']=np.nan
data['Uber Motor']=np.nan
data['Gojek Car']=np.nan
data['Grab Car']=np.nan
data['Uber Car']=np.nan
data['Grab Taxi']=np.nan
...
for i in data.index:
    ...
    if(data['Gojek - pernah'][i] not in [np.nan]):
        data['Gojek Motor'][i]=1
    else:
        if(data['Gojek - tidak pernah'][i] not in [np.nan]):
            data['Gojek Motor'][i]=2
        else:
            data['Gojek Motor'][i]=0
    
    if(data['Uber motor - pernah'][i] not in [np.nan]):
        data['Uber Motor'][i]=1
    else:
        if(data['Uber motor - tidak pernah'][i] not in [np.nan]):
            data['Uber Motor'][i]=2
        else:
            data['Uber Motor'][i]=0
    
    if(data['Grabbike - pernah'][i] not in [np.nan]):
        data['Grab Motor'][i]=1
    else:
        if(data['Grabbike - tidak pernah'][i] not in [np.nan]):
            data['Grab Motor'][i]=2
        else:
            data['Grab Motor'][i]=0
            
    if(data['GoCar - pernah'][i] not in [np.nan]):
        data['Gojek Car'][i]=1
    else:
        if(data['GoCar - tidak pernah'][i] not in [np.nan]):
            data['Gojek Car'][i]=2
        else:
            data['Gojek Car'][i]=0
    
    if(data['Grab Car - pernah'][i] not in [np.nan]):
        data['Grab Car'][i]=1
    else:
        if(data['Grab Car - tidak pernah'][i] not in [np.nan]):
            data['Grab Car'][i]=2
        else:
            data['Grab Car'][i]=0
    
    if(data['Uber Car - pernah'][i] not in [np.nan]):
        data['Uber Car'][i]=1
    else:
        if(data['Uber Car - tidak pernah'][i] not in [np.nan]):
            data['Uber Car'][i]=2
        else:
            data['Uber Car'][i]=0
    
    if(data['Grab Taxi - pernah'][i] not in [np.nan]):
        data['Grab Taxi'][i]=1
    else:
        if(data['Grab Taxi - tidak pernah'][i] not in [np.nan]):
            data['Grab Taxi'][i]=2
        else:
            data['Grab Taxi'][i]=0
...
data_fixed=data[[
            ...
            'Gojek Motor','Grab Motor','Uber Motor','Gojek Car','Uber Car','Grab Car','Grab Taxi'
        ]]
